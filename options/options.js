...
// Update the room name based on the value in the textbox.
...
async function updateFaviconOption() {
  try {
    await browser.storage.local.set({
      faviconOption: document.querySelector('input[name="favicon-option"]:checked').value,
    });
  } catch (error) {
...
document.querySelector('#room-name-update').addEventListener('click', updateRoomName);
document.querySelectorAll('input[name="favicon-option"]').forEach(input =>
  input.addEventListener('change', updateFaviconOption)
);
